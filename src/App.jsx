...
const contentItems=[
  {title:"IBM Accepts the Advisory",type:"win",desc:"IBM accepted our security advisory for their MCP Gateway, an SSRF plus credential-forwarding flaw in their AI-agent infrastructure, credited to us. That sits on top of three published CVEs (Gitea, File Browser rated High, and OpenBao, found by watching a fixed Vault bug that still lived in the fork) and an accepted host file read-write finding at Incus. Real vulnerabilities in software used by millions. Proof, not marketing.",link:"https://securva.net/research/disclosures",accent:"#FF6B6B"},
...
const projects=[
  ...
  {name:"Securva CVE Pipeline",desc:"Published CVEs credited to us: CVE-2026-27761 (Gitea), CVE-2026-55667 (File Browser, rated High), and CVE-2026-63131 (OpenBao, Moderate). Accepted advisories now in remediation: IBM MCP Gateway (GHSA-3frw-wjxx-2p6m, SSRF plus credential forwarding) and Incus (arbitrary host file read-write), with more in disclosure. Self-hosted OSS audits via incomplete-fix and cross-fork methodology, web2 and web3.",tech:["CVEs","IBM","Methodology"],status:"Live",accent:"#FF6B6B"},
    ...
    <>
      ...
      <div style={{backgroundColor:T.bg,minHeight:"100vh",color:T.text,fontFamily:"'DM Sans', sans-serif",transition:"background-color 0.4s, color 0.4s"}}>
        ...
        <section id="building" ref={divRef} style={{padding:"90px 24px",maxWidth:1060,margin:"0 auto",scrollMarginTop:60}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:14,justifyContent:"center"}}>
            {divisions.map((d,i)=>{const ref=useRef(null);const glow=useMouseGlow(ref);const[vR,inV]=useInView(0.05);const[h,setH]=useState(false);
              const CardTag = d.url ? "a" : "div";
              const linkProps = d.url ? {href:d.url,target:"_blank",rel:"noopener noreferrer"} : {};
              return <div key={d.name} ref={vR} style={{flex:"1 1 240px",maxWidth:280}}>
                <CardTag {...linkProps} ref={ref} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{position:"relative",background:T.card,border:`1px solid ${h?d.accent:T.border}`,borderRadius:18,padding:"28px 22px 24px",overflow:"hidden",opacity:inV?1:0,transform:inV?"translateY(0)":"translateY(36px)",transition:`all 0.7s cubic-bezier(0.16, 1, 0.3, 1) ${i*0.1}s`,cursor:d.url?"pointer":"default",textDecoration:"none",display:"block"}}>
                  ...
                  {!d.url&&<span style={{fontFamily:"'IBM Plex Mono', monospace",fontSize:9,color:T.gray,letterSpacing:1.5,marginTop:10,display:"block",textTransform:"uppercase"}}>Coming Soon</span>}
                </CardTag>
              </div>})}
          </div>
        ...
        <div id="privacy-modal" onClick={e=>{if(e.target.id==="privacy-modal")e.target.style.display="none"}} style={{display:"none",position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(8px)",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{background:dark?"#0a0a0a":"#faf8f4",border:`1px solid ${T.border}`,borderRadius:16,padding:"32px 28px",maxWidth:500,width:"100%",maxHeight:"80vh",overflowY:"auto",position:"relative"}}>
            ...
            <h3 style={{fontFamily:"'Cormorant Garamond', serif",fontSize:22,fontWeight:700,color:T.text,marginBottom:16}}>Privacy Policy</h3>
            <div style={{fontFamily:"'DM Sans'",fontSize:13,color:T.gray,lineHeight:1.85}}>
              <p style={{marginBottom:12}}>This website (babakizo.com) collects minimal data to improve your browsing experience. We use cookies for analytics and site functionality.</p>
              <p style={{marginBottom:12}}><strong style={{color:T.text}}>Data Collected:</strong> Basic analytics (page views, session duration). No personal information is sold or shared with third parties.</p>
              <p style={{marginBottom:12}}><strong style={{color:T.text}}>Your Rights (NDPA 2023):</strong> Under the Nigeria Data Protection Act 2023, you have the right to access, correct, or request deletion of your personal data. You may withdraw consent at any time by clearing your browser cookies.</p>
              <p style={{marginBottom:12}}><strong style={{color:T.text}}>Contact:</strong> For data-related inquiries, reach out via any channel listed in the Connect section above.</p>
              <p style={{fontFamily:"'IBM Plex Mono', monospace",fontSize:10,color:T.gold,letterSpacing:2,marginTop:20}}>Last updated: {new Date().getFullYear()}</p>
            </div>
          </div>
        </div>
      </div>
